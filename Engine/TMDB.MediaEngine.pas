...
begin
  ...
  try
    ...
    begin
      ...
      if Assigned(LArr) and (LArr.Count > 0) then
        Result.LocalProducao := MapearPais((LArr.Items[0] as TJSONObject).GetValue<string>('name', '')) + Char(160) + FormatarParaTag(MapearPais((LArr.Items[0] as TJSONObject).GetValue<string>('name', '')))
      else
      begin
        LOriginArr := LJson.GetValue<TJSONArray>('origin_country', nil);
        if Assigned(LOriginArr) and (LOriginArr.Count > 0) then
          Result.LocalProducao := MapearPais(LOriginArr.Items[0].Value) + Char(160) + FormatarParaTag(MapearPais(LOriginArr.Items[0].Value));
      end;
