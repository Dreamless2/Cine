  ...
  try
    ...
    begin
      ...
      begin
        ...
        else Result.Tags := '#Serie ' + FormatarParaTag('Serie' + LAno) + Chr(160) + '#Série' + FormatarParaTag('Série' + LAno);
      end;
    end;
    ...
    begin
      ...
      if Assigned(LArr) and (LArr.Count > 0) then
        Result.LocalProducao := GerarTag(FormatarParaTag(MapearPais((LArr.Items[0] as TJSONObject).GetValue<string>('name', ''))))
      else
      begin
        LOriginArr := LJson.GetValue<TJSONArray>('origin_country', nil);
        if Assigned(LOriginArr) and (LOriginArr.Count > 0) then
          Result.LocalProducao := GerarTag(FormatarParaTag(MapearPais(LOriginArr.Items[0].Value)));
      end;
